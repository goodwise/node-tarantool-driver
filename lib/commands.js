...
Commands.prototype._getRequestId = function _getRequestId (){
	var _id = this._id
	if (_id[0] > maxSmi)
		_id[0] = 1;
	return _id[0]++;
...
Commands.prototype._getSpaceId = function _getSpaceId (name){
	var _this = this;
...
Commands.prototype._getIndexId = function _getIndexId (spaceId, indexName){
	var _this = this;
...
Commands.prototype._getMetadata = function _getMetadata (spaceName, indexName){
	var _this = this;
...
Commands.prototype.ping = function ping (opts = {}){
	var reqId = this._getRequestId();
	var len = 9;
...
Commands.prototype.begin = function begin (transTimeoutSec = 60.01 /* prevent JS from converting 60.0 to 60 */, isolationLevel = 0, opts = {}) {
	if (!this.streamId) {
		reject(
...
Commands.prototype.commit = function commit (opts = {}) {
	if (!this.streamId) {
		reject(
			new TarantoolError('Cannot find streamId, maybe called method outside of transaction?')
		);
...
Commands.prototype.rollback = function rollback (opts = {}) {
	if (!this.streamId) {
		reject(
			new TarantoolError('Cannot find streamId, maybe called method outside of transaction?')
		);
...
Commands.prototype.selectCb = function selectCb (spaceId, indexId, limit, offset, iterator, key, success, error, opts = {}){
...
Commands.prototype.delete = function _delete (spaceId, indexId, key, opts = {}){
	var _this = this;
...
Commands.prototype.update = function update (spaceId, indexId, key, ops, opts = {}){
	if (Number.isInteger(key)) key = [key];
...
Commands.prototype.upsert = function upsert (spaceId, ops, tuple, opts = {}){
	var _this = this;
...
Commands.prototype.eval = function eval (expression, opts = {}){
...
Commands.prototype.call = function call (functionName, opts = {}){
...
Commands.prototype.sql = function sql (query, bindParams = [], opts = {}){
	var reqId = this._getRequestId();
...
Commands.prototype.prepare = function prepare (query, opts = {}){
	var reqId = this._getRequestId();
...
Commands.prototype.id = function id (version = 3, features = [1], auth_type = 'chap-sha1', opts){
	var reqId = this._getRequestId();
...
Commands.prototype.insert = function insert (spaceId, tuple, opts = {}){
	var reqId = this._getRequestId();
	return this._replaceInsert(tarantoolConstants.RequestCode.rqInsert, reqId, spaceId, tuple, opts);
};

Commands.prototype.replace = function replace (spaceId, tuple, opts = {}){
	var reqId = this._getRequestId();
	return this._replaceInsert(tarantoolConstants.RequestCode.rqReplace, reqId, spaceId, tuple, opts);
};

Commands.prototype._replaceInsert = function _replaceInsert (cmd, reqId, spaceId, tuple, opts = {}){
	...
	if (typeof(spaceId)=='string')
	{
		return this._getMetadata(spaceId, 0)
			.then(function(info){
				return _this._replaceInsert(cmd, reqId, info[0], tuple, opts);
			})
	}
	var bufTuple = packr.encode(tuple);
...
Commands.prototype._auth = function _auth (username, password){
	var _this = this;
...
function scramble(password, salt){
	...
	var step3 = shatransform(
		Buffer.concat(
			[
				bufferSubarrayPoly.apply(encSalt, [0, 20]),
				step2
			]
		)
	);
