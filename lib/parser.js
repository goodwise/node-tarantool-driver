...
exports.processResponse = function(headers, data){
  ...
  if (!dfd) {
    return this.errorHandler(
      new TarantoolError(
        `Request with id ${reqId} was not found, maybe a duplicate`
      )
    );
  }
  if (code === 0) {
    ...
    case RequestCode.rqDelete:
      if (task[4]) {
        ...
        if (!this.namespace[task[2][0]]) {
          const err = new TarantoolError('Failed to convert tuple to object');
          err.tuples = data[KeysCode.data];
          throw err;
        }
